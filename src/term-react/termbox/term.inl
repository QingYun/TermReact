...
static char *read_file(const char *file) {
	...
	char *data = static_cast<char*>(malloc(st.st_size));
	if (!data) {
		fclose(f);
		return 0;
	}
...
static int init_term(void) {
	...
	keys = static_cast<const char**>(malloc(sizeof(const char*) * (TB_KEYS_NUM+1)));
	...
	funcs = static_cast<const char**>(malloc(sizeof(const char*) * T_FUNCS_NUM));
